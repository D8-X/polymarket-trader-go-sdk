...
func TestPrepareAndSignAmounts(t *testing.T) {
	...
	cases := []struct {
		...
		{"buy tick 0.01", BUY, "0.01", 0.55, 10, "5500000", "10000000"},
		{"sell tick 0.01", SELL, "0.01", 0.55, 10, "10000000", "5500000"},
		{"buy tick 0.001", BUY, "0.001", 0.555, 10, "5550000", "10000000"},
		{"buy tick 0.0001", BUY, "0.0001", 0.5555, 10, "5555000", "10000000"},
		{"buy tick 0.1", BUY, "0.1", 0.5, 5, "2500000", "5000000"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			signed, err := ob.PrepareAndSign("100", tc.side, OrderTypeGTC, tc.price, tc.size, "k", OrderOpts{TickSize: tc.tick})
			if err != nil {
