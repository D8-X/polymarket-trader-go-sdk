...
func newMockCLOBServer(t *testing.T, handler func(w http.ResponseWriter, r *http.Request)) (*CLOBClient, func()) {
	t.Helper()
...
func TestGetMarketsRoundTrip(t *testing.T) {
	c, cleanup := newMockCLOBServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(PaginatedResponse[MarketInfo]{
			Data:       []MarketInfo{{ConditionID: "0xc0", Question: "Q", Active: true}},
			NextCursor: "next",
			Count:      1,
			Limit:      1000,
		})
...
func TestGetMarketRoundTrip(t *testing.T) {
	c, cleanup := newMockCLOBServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(MarketInfo{ConditionID: "0xabc", Question: "Q"})
	})
	defer cleanup()
...
func TestGetMarketByTokenRoundTrip(t *testing.T) {
	c, cleanup := newMockCLOBServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(MarketByTokenInfo{
			ConditionID:      "0xc1",
...
func TestGetSamplingSimplifiedMarketsRoundTrip(t *testing.T) {
	c, cleanup := newMockCLOBServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(PaginatedResponse[SimplifiedMarketInfo]{
			Data: []SimplifiedMarketInfo{{ConditionID: "0xdead"}},
		})
	})
	defer cleanup()
